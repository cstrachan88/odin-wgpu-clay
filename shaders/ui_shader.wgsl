...
fn fs_main(in: Vertex_Output) -> @location(0) vec4f {
    ...
    if (any(within_radius) && (
        (within_radius.x && corner_distances.x > in.corner_radius.x) || // Top-left check
        (within_radius.y && corner_distances.y > in.corner_radius.y) || // Top-right check
        (within_radius.z && corner_distances.z > in.corner_radius.z) || // Bottom-left check
        (within_radius.w && corner_distances.w > in.corner_radius.w)    // Bottom-right check
    )) { discard; }
